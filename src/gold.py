...
def _create_gold_table(df: pd.DataFrame, table_name: str, if_exists: str = "replace"):
    ...
    logger.info(f"✅ {GOLD_SCHEMA}.{table_name} — {len(df)} rows loaded")
...
def create_daily_revenue():
    ...
    try:
        ...
        logger.error(f"Error while creating daily revenue: {e}")
        raise
...
def create_product_performance():
    ...
    try:
        ...
        logger.info(f"✅ Gold table 'product_performance' created with {len(df)} rows")
    except Exception as e:
        logger.error(f"Error while creating product performance: {e}")
        raise
...
def create_customer_ltv():
    ...
    try:
        ...
        logger.info(f" Gold table 'customer_ltv' created with {len(df)} rows")
    except Exception as e:
        logger.error(f"Error while creating customer LTV: {e}")
        raise
...
def create_gold_layer():
    ...
    try:
        ...
        logger.error(f"Error while creating GOLD layer: {e}")
        raise
